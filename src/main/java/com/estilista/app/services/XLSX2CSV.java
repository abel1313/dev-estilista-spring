...
//        }
    	...
    	String d = "124.15";
    	boolean isValid = d.split(",").length > 1;
    	String replace = ""; 
    	if(isValid) {
    		replace = d.replace(",", ".");
    	}else {
    	...
    	System.err.println(isValid + " "+ replace );
    	Pattern pa = Pattern.compile("^[1-9]{0,1}\\d{0,9}(.\\d{0,2})*");
    	Matcher m = pa.matcher("12121");
    	System.err.println(m.matches() + " Si fue ");
    
        File xlsxFile = new File("Z:\\Documentos\\Ejemplos excel\\Libro.xlsx");
        if (!xlsxFile.exists()) {
